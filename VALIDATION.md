...
## 2026-08-20 v12.7.0正式Release复核

- 发布范围为`v12.5.0..v12.7.0`的通用源码、Schema、测试和文档，新增新会话
  Session Doctor、已安装Skill漂移检查，以及AutoCAD D2L 2.2正式梁文字证据后备。
- AutoCAD D2L现场构建引用目标宿主Autodesk程序集，不提交DLL。AutoCAD 2023真实梁图
  在相同展开根下与中望的24910条直接/递归文字组合键双向差集为0，顶层实体、直接文字、
  属性和块实例计数一致，跳过/截断为0；LINE/POLYLINE仍有8条净差，不声明全实体等价。
- 2020/2018/2014继续只声明共享源码、x64和DWG版本门禁；对应宿主实机验证尚未完成。
  AutoCAD 2014仍拒绝AC1032，不自动降版转换。
- 发布前根测试`67 passed`、Skill测试`59 passed`、深化模块`723 passed, 8 skipped`；
  48个PowerShell脚本在PowerShell 7和Windows PowerShell 5.1解析错误均为0。
- 发布内容不包含DWG/DXF、真实工程结果、DLL/PDB/EXE、许可证、凭证、客户证据、
  固定个人路径、缓存或临时构建目录。GitHub Release不附加本机二进制资产，只使用源码归档。
- D4跨项目自动识别继续冻结。D2L后备只补齐正式梁文字证据入口，不将既有单项目候选提升为
  跨项目自动确认，也不构成专业审图或生产放行。
