...
function Add-Candidate {
    ...
    if (@($Rows | Where-Object { $_.dedupe_key -eq $key }).Count -gt 0) { return }
    ...
    if ($CandidateVendor -eq 'ZWCAD') {
        ...
        $console = if (Test-Path -LiteralPath $consolePath -PathType Leaf) { $consolePath } else { $null }
    ...
    $version = $null
    try { $version = (Get-Item -LiteralPath $exe).VersionInfo.FileVersion } catch { $version = $null }
...
if ($Vendor -in @('Any', 'ZWCAD')) { Add-Candidate $rows 'ZWCAD' $env:CAD_ZWCAD_ROOT 'environment' }
if ($Vendor -in @('Any', 'AutoCAD')) { Add-Candidate $rows 'AutoCAD' $env:CAD_AUTOCAD_ROOT 'environment' }
$uninstallRoots = @(
...
foreach ($registryRoot in $uninstallRoots) {
    foreach ($item in @(Get-ItemProperty -Path $registryRoot -ErrorAction SilentlyContinue)) {
        ...
        if ($Vendor -in @('Any', 'ZWCAD') -and $name -match '(?i)ZWCAD|中望CAD') { Add-Candidate $rows 'ZWCAD' $location 'registry-uninstall' }
        if ($Vendor -in @('Any', 'AutoCAD') -and $name -match '(?i)AutoCAD') { Add-Candidate $rows 'AutoCAD' $location 'registry-uninstall' }
    }
}
$searchRoots = @(
    $env:ProgramFiles, ${env:ProgramFiles(x86)}, 'C:\Program Files',
    'D:\Program Files', 'E:\Program Files', 'F:\Program Files', 'G:\Program Files'
) | Where-Object { $_ -and (Test-Path -LiteralPath $_ -PathType Container) } | Select-Object -Unique
foreach ($searchRoot in $searchRoots) {
    if ($Vendor -in @('Any', 'ZWCAD')) {
        foreach ($exe in @(Get-ChildItem -LiteralPath $searchRoot -Filter 'ZWCAD.exe' -File -Recurse -Depth 4 -ErrorAction SilentlyContinue)) { Add-Candidate $rows 'ZWCAD' $exe.DirectoryName 'filesystem' }
    }
    if ($Vendor -in @('Any', 'AutoCAD')) {
        foreach ($exe in @(Get-ChildItem -LiteralPath $searchRoot -Filter 'acad.exe' -File -Recurse -Depth 4 -ErrorAction SilentlyContinue)) { Add-Candidate $rows 'AutoCAD' $exe.DirectoryName 'filesystem' }
    }
}
$result = @(
    $rows |
...
if ($RequireCurrentBackend) {
    ...
    if ($result.Count -eq 0) {
        throw 'No current runnable backend was found. The released exporter backend is ZWCAD-only; AutoCAD discovery is not a validated AutoCAD backend.'
    }
}
